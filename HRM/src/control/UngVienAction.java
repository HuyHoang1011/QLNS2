...
public class UngVienAction implements ActionListener{
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(str.equals("Thêm")) {
			...
			if(!CHECK.checkUngVienData(ungVienData)){
				...
				if(mucLuongDeal<bctd.getMucLuongToiThieu() ){
				 ...
				 if (!isMaUngVienValid(ungVienData[1])) {
				        JOptionPane.showMessageDialog(temp, "Mã ứng viên phải có dạng xxx, ví dụ 001", "Lỗi", JOptionPane.ERROR_MESSAGE);
				        return;
				    }
	 ...
	 public boolean isMaUngVienValid(String maUngVien) {
	              String regex = "\\d{3}";
	        return maUngVien.matches(regex);
	    }
