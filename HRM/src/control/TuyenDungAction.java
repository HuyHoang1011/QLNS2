...
//	private ThreadOne a;
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(str.equals("Thêm")) {
			...
			if(!isMaTuyenDungValid(dataBCTD[0])) {
				 JOptionPane.showMessageDialog(temp, "Mã tuyển dụng phải có dạng'BCTDxxx'!", "Lỗi", JOptionPane.ERROR_MESSAGE);
			        return; 
			}
			...
			if(Double.parseDouble(dataBCTD[6]) > Double.parseDouble(dataBCTD[7]))  {
				 JOptionPane.showMessageDialog(temp, "Lương tối thiểu phải bé hơn lương tối đa !", "Lỗi", JOptionPane.ERROR_MESSAGE);
			        return; 
			}
...
//				i.getHanNopHoSo().getDayOfMonth()==Integer.parseInt(temp.getTuyenDung_Find().getText())||
	...
	public boolean isMaTuyenDungValid(String maTuyenDung) {
	   
	    if (maTuyenDung.length() != 7) {
	        return false;
	    }

	    if (!maTuyenDung.substring(0, 4).equals("BCTD")) {
	        return false;
	    }

	    String soDuoi = maTuyenDung.substring(4);
	    try {
	        Integer.parseInt(soDuoi);
