	...
	public void actionPerformed(ActionEvent e) {
		...
		if(temp.getDepartmentAdd().getActionType()==ACTION_TYPE.ADD) {
			...
			if(CHECK.checkDepartmentData(app, data)) {
				...
				if(access_PHONGBAN.isMaPhongExists(phongBan.getTenPhong()) && access_PHONGBAN.isMaPhongExists(phongBan.getMaPhong())) {
					app.showMessage("Mã phòng ban và tên phòng ban này đã tồn tại!");
					return;
				}
				else if(access_PHONGBAN.isMaPhongExists(phongBan.getMaPhong())) {
					app.showMessage("Mã phòng ban này đã tồn tại!");
					return;
				}else if(access_PHONGBAN.isMaPhongExists(phongBan.getTenPhong())) {
					app.showMessage("Tên phòng ban này đã tồn tại!");
					return;
				}
